...
def data_prepare (candle_seq_len = 64):
    ...
    return vocab_size, train_data, val_data, train_targets, val_targets, index_to_token, token_weights
...
def train(model, optimizer, scaler, loss_fn, train_loader, val_loader, writer, device, index_to_candle, hyperparams):
    ...
    try:
        for epoch in range(start_epoch,num_epochs):
            # Train the model
            optimizer = set_learning_rate(optimizer, l_rates[epoch], weight_decay=0)
            ...
            log_str = f'Training time: {formatted_time}, Epoch [{epoch + 1}/{num_epochs}], ' + \
                  ...
                  f'Validation accuracy: {best_accuracy_mark}{accuracy:.2f}%{RESET}, Learning rate: {lr:.2e} '
            ...
            #function converts tockns to text and write to tensorboard
            def decode_and_write(writer, model, vlabels, voutputs, epoch, index_to_candle):
                vout_tockens = voutputs.softmax(dim=1).argmax(dim=1)
                vout_tockens = vout_tockens.cpu()
                #decode tockens to text
                vout_text = []
                vlabels_text = []
            ...
            if vepoch_loss/max_val_batches < best_vloss:
                ...
                time_since_start = int(time.perf_counter() - start_time)
                formatted_time = f'{time_since_start//3600:02d}:{(time_since_start//60)%60:02d}:{time_since_start%60:02d}'
                log_str = f'Training time: {formatted_time}, The best model saved at epoch {epoch+1} with validation loss {best_vloss_mark}{vepoch_loss/max_val_batches:.6f}{RESET} Learning rate: {lr:.2e} Weight decay: {wd:.2e} '
                print(log_str)
...
def main():
    ...
    loss_fn = nn.CrossEntropyLoss(weight=token_weights, label_smoothing=0.1)
